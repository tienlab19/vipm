...
cp "$ROOT/vipm/questions.json" "$OUTPUT/questions.json"
...
if grep -En 'JSONQuestionBankRepository|UserDefaultsStudyProgressRepository|UserDefaults|@AppStorage|Bundle|QuestionDTO|QuestionBankDTO' "$ROOT"/vipm/Presentation/{Views,ViewModels}/*.swift; then
...
"$OUTPUT/check" "$ROOT/vipm/questions.json"
printf '\nChecks with DEBUG\n'
xcrun swiftc -D DEBUG -parse-as-library "${SOURCES[@]}" -o "$OUTPUT/check-debug"
"$OUTPUT/check-debug" "$ROOT/vipm/questions.json"
